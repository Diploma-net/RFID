...
export default function TableForReports({ data, textForButton }) {
    ...
    <Table striped style={{ marginTop: "50px" }}>
      <thead>
        <tr>
          ...
          <th style={{textAlign: "center"}}>Дата проверки</th>
        </tr>
      </thead>
      <tbody>
        {Array.from(data.entries()).map(([key, value]) => (
          <tr key={key}>
            ...
            <td style={{textAlign: "center"}}>
              <Button
                onClick={() => navigate(`/report/${value.idReport}`, { replace: false })}
