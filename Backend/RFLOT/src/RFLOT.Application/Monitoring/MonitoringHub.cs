...
public class MonitoringHub : Hub, INotificationHandler<NewEquipCheckedInReport>
    {
        private readonly List<(string, List<PlaneConnection>)> _planeConnections = new List<(string, List<PlaneConnection>)>();
        ...
        public async Task JoinInMonitoringZone(ZoneConnection connection)
        {
            ...
            {
                _zoneConnections.Add(( $"{connection.IdReport.ToString()}_{connection.IdZone.ToString()}", new List<ZoneConnection> { connection }));
            }
            await Groups.AddToGroupAsync(Context.ConnectionId, $"{connection.IdReport.ToString()}_{connection.IdZone.ToString()}");
        }
        ...
        public async Task Handle(NewEquipCheckedInReport notification, CancellationToken cancellationToken)
        {
            if (Clients != null)
            {
                ...
                }).SendCoreAsync("NewEquipCheck", new object[] { new Message(notification.Space, notification.Status) }, cancellationToken);
            }
        }
    }
