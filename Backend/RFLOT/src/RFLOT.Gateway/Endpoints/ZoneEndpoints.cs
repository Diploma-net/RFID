...
public static class ZoneEndpoints
{
    public static WebApplication AddZoneEndpoints(this WebApplication app)
    {
        ...
        endpoints.MapPost("/start-check",
            ...
            {
                var oneZoneInfo = await mediator.Send(new StartCheckZoneCommand(startCheckDto.IdZone,
                    startCheckDto.IdReport, startCheckDto.IdUser));
                return Results.Ok(
                    new
                    {
        ...
        endpoints.MapPost("/stop-check",
            ...
            {
                await mediator.Send(new StopCheckZoneCommand(stopCheckDto.IdZone, stopCheckDto.IdReport,
                    stopCheckDto.IdUser));
                return Results.Ok();
            });
        return app;
